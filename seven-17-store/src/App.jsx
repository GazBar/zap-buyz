                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    {!isLogin && (
                        <div>
                            ...
                            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500 sm:text-sm" />
                        </div>
                    )}
                    <div>
                        <label htmlFor="email" className="block text-sm font-medium text-gray-700">Email Address</label>
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500 sm:text-sm" />
                    </div>
                    <div>
                        <label htmlFor="password" className="block text-sm font-medium text-gray-700">Password</label>
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500 sm:text-sm" />
                    </div>
                    <div>
                        <button type="submit" className="flex items-center justify-center w-full px-4 py-3 font-semibold text-gray-900 transition-colors duration-200 bg-lime-500 border border-transparent rounded-md shadow-sm hover:bg-lime-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-lime-500">
...
const AccountPage = ({ user, setModal, setCurrentPage }) => {
    ...
    const handleUpdateProfile = async (e) => { e.preventDefault(); try { if (auth.currentUser.displayName !== name) { await updateProfile(auth.currentUser, { displayName: name }); } if (auth.currentUser.email !== email) { await updateEmail(auth.currentUser, email); } await setDoc(doc(db, "users", user.uid), { name, email }, { merge: true }); setModal({ title: 'Success', message: 'Your profile has been updated.' }); } catch (error) { setModal({ title: 'Error', message: `Could not update profile: ${error.message}` }); } };
    const handlePasswordReset = async () => { try { await sendPasswordResetEmail(auth, user.email); setModal({ title: 'Password Reset', message: 'A password reset link has been sent to your email.' }); } catch (error) { setModal({ title: 'Error', message: error.message }); } };
    const handleLogout = async () => { await signOut(auth); setCurrentPage('home'); };

    return ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 mx-auto max-w-4xl sm:p-6 lg:p-8"><div className="bg-white rounded-lg shadow-lg overflow-hidden"><div className="p-8"><h2 className="text-3xl font-bold text-gray-900" style={{ fontFamily: "'Poppins', sans-serif" }}>My Account</h2><p className="mt-2 text-gray-600">Manage your orders and account details.</p></div><div className="border-b border-gray-200"><nav className="-mb-px flex px-8" aria-label="Tabs"><button onClick={() => setActiveTab('orders')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'orders' ? 'border-lime-500 text-lime-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}><Package className="inline-block w-5 h-5 mr-2"/>Order History</button><button onClick={() => setActiveTab('settings')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'settings' ? 'border-lime-500 text-lime-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}><Settings className="inline-block w-5 h-5 mr-2"/>Account Settings</button></nav></div><div className="p-8">{activeTab === 'orders' && ( <div>{loadingOrders ? <p>Loading orders...</p> : orders.length > 0 ? ( <div className="space-y-4">{orders.map(order => ( <div key={order.id} className="border rounded-lg p-4"><div className="flex justify-between items-center"><p className="font-semibold">Order #{order.id.substring(0, 8)}</p><p className="text-sm text-gray-500">{new Date(order.createdAt.seconds * 1000).toLocaleDateString()}</p></div><div className="mt-4">{order.items.map(item => ( <div key={item.id} className="flex items-center justify-between py-2 border-b last:border-b-0"><p>{item.name} (x{item.quantity})</p><p>£{(item.price * item.quantity).toFixed(2)}</p></div> ))}<p className="text-right font-bold mt-2">Total: £{order.total.toFixed(2)}</p></div></div> ))}</div> ) : <p>You have no past orders.</p>}</div> )}{activeTab === 'settings' && ( <form onSubmit={handleUpdateProfile} className="space-y-6 max-w-lg mx-auto"><div><label htmlFor="name" className="block text-sm font-medium text-gray-700">Full Name</label><input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500 sm:text-sm" /></div><div><label htmlFor="email" className="block text-sm font-medium text-gray-700">Email Address</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500 sm:text-sm" /></div><div><button type="submit" className="w-full px-4 py-2 font-semibold text-gray-900 bg-lime-500 rounded-md hover:bg-lime-600">Update Profile</button></div><div className="text-center"><button type="button" onClick={handlePasswordReset} className="text-sm text-lime-600 hover:underline">Send Password Reset Email</button></div><div className="border-t pt-6"><button type="button" onClick={handleLogout} className="w-full flex items-center justify-center px-4 py-2 font-semibold text-white bg-gray-700 rounded-md hover:bg-gray-800"><LogOut className="w-5 h-5 mr-2" /> Log Out</button></div></form> )}</div></div></motion.div> ); };
...
const AdminPage = ({ user, products, setProducts, setModal }) => {
    ...
    useEffect(() => {
        const fetchMessages = async () => { setLoading(true); try { const messagesQuery = query(collection(db, "messages"), orderBy("createdAt", "desc")); const querySnapshot = await getDocs(messagesQuery); const messagesData = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })); setMessages(messagesData); } catch (error) { console.error("Error fetching messages: ", error); } finally { setLoading(false); } };
        if (activeTab === 'messages') fetchMessages();
    }, [activeTab]);
    
    const ProductForm = ({ product, onClose }) => {
        const [formData, setFormData] = useState(product || { name: '', description: '', price: 0, category: 'Spring', image: '', rating: 0, reviews: 0 });
        const handleChange = (e) => { const { name, value } = e.target; setFormData(prev => ({ ...prev, [name]: name === 'price' || name === 'rating' || name === 'reviews' ? parseFloat(value) || 0 : value })); };
        ...
        const handleFormSubmit = async (e) => {
            ...
            try {
                if (product && product.id) {
                    ...
                    setModal({ title: 'Success', message: 'Product added successfully.' });
                }
                refreshProducts();
                onClose();
            } catch (error) {
                setModal({ title: 'Error', message: 'Failed to save product.' });
        ...
        return (
            <form onSubmit={handleFormSubmit} className="space-y-4">
                 ...
                 <div><label>Image URL</label><input name="image" value={formData.image} onChange={handleChange} required className="w-full p-2 border rounded-md"/></div>
                 ...
                 <button type="submit" className="w-full px-4 py-2 font-semibold text-white bg-lime-500 rounded-md hover:bg-lime-600">Save Product</button>
            </form>
        );
    };
    ...
    const handleDeleteProduct = async (productId) => {
        if (window.confirm('Are you sure you want to delete this product?')) {
            try {
                ...
                setModal({ title: 'Success', message: 'Product deleted.' });
                refreshProducts();
            } catch (error) {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="p-4 mx-auto max-w-7xl sm:p-6 lg:p-8">
            <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                <div className="p-8"><h2 className="text-3xl font-bold text-gray-900" style={{ fontFamily: "'Poppins', sans-serif" }}>Admin Panel</h2></div>
                <div className="border-b border-gray-200"><nav className="-mb-px flex px-8"><button onClick={() => setActiveTab('messages')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'messages' ? 'border-lime-500 text-lime-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><MessageSquare className="inline-block w-5 h-5 mr-2"/>Messages</button><button onClick={() => setActiveTab('products')} className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'products' ? 'border-lime-500 text-lime-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}><Package className="inline-block w-5 h-5 mr-2"/>Manage Products</button></nav></div>
                <div className="p-8">
                    {activeTab === 'messages' && ( <div>{loading ? <p>Loading messages...</p> : messages.length > 0 ? ( <div className="space-y-4">{messages.map(msg => ( <div key={msg.id} className="p-4 border rounded-md bg-gray-50"><div className="flex justify-between items-start"><div><p className="font-semibold">{msg.name} <span className="font-normal text-gray-500">- {msg.email}</span></p><p className="mt-2 text-gray-700">{msg.message}</p></div><a href={`https://mail.google.com/mail/?view=cm&fs=1&to=${msg.email}&su=Re: Your message to Seven 17`} target="_blank" rel="noopener noreferrer" className="flex items-center px-3 py-1 text-sm font-semibold text-white bg-lime-500 rounded-md hover:bg-lime-600 transition-colors"><Mail className="w-4 h-4 mr-2"/> Reply</a></div><p className="mt-2 text-xs text-gray-400">Received: {msg.createdAt ? new Date(msg.createdAt.seconds * 1000).toLocaleString() : 'No date'}</p></div> ))}</div> ) : <p>No messages yet.</p>}</div> )}
                    {activeTab === 'products' && (
                        <div>
                            <div className="flex justify-end mb-4">
                                <button onClick={() => { setEditingProduct(null); setIsProductModalOpen(true); }} className="flex items-center px-4 py-2 font-semibold text-white bg-lime-500 rounded-md hover:bg-lime-600"><PlusCircle className="w-5 h-5 mr-2"/>Add Product</button>
                            </div>
                                ...
                                {products.map(p => (
                                    <div key={p.id} className="p-4 border rounded-lg bg-gray-50">
                                        ...
                                        <div className="flex justify-end space-x-2 mt-4">
                                            <button onClick={() => { setEditingProduct(p); setIsProductModalOpen(true); }} className="p-2 text-blue-600 hover:bg-blue-100 rounded-full"><Edit className="w-5 h-5"/></button>
                                            <button onClick={() => handleDeleteProduct(p.id)} className="p-2 text-red-600 hover:bg-red-100 rounded-full"><Trash2 className="w-5 h-5"/></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <AnimatePresence>
                {isProductModalOpen && (
                    <Modal title={editingProduct ? 'Edit Product' : 'Add New Product'} onClose={() => setIsProductModalOpen(false)}>
                        <ProductForm product={editingProduct} onClose={() => setIsProductModalOpen(false)} />
                    </Modal>
                )}
            </AnimatePresence>
        </motion.div>
